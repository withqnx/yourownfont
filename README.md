...
**최소 단어 세트(다벌):** 기계적인 격자 대신, **재미있는 실제 단어**(달빛·꿈·행복·닭·값 …)를
씁니다. 이 단어들은 그리디 집합피복으로 골라져 **모든 초성을 세로·가로모음 두 문맥에서,
모든 중성·종성을 한 번씩** 담습니다(현재 49단어·76음절, `charset.WORDS`,
`charset.verify_coverage`로 검증). 조합 시 대상 글자에 맞는 벌을 골라 씁니다.

**규칙적인 부분은 수학으로:** 받침이 있으면 초성+중성 블록을 세로로 압축하는데, 그 비율은
훈민정음 글꼴 기하 연구(1996)의 실측치(≈0.756)를 사용합니다 — 그래서 "받침 있는 중성"을
따로 쓸 필요가 없습니다. 템플릿은 여러 쪽이며, 스캔한 모든 쪽을 함께 업로드합니다.
...
- 배치는 **획 굵기 보존**(등방에 가까운 스케일 + 중심 배치, `hangul._MAX_ANISO`)으로
  늘어짐을 줄였음. 다만 **복합모음(ㅘ, ㅟ …)** 은 사각형 슬라이스라 추출이 여전히 근사적 →
  중심-클러스터 배치·자음별 메트릭(논문) 적용이 다음 개선점.
